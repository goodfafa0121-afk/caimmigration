...
async function saveImages(images: string[]) {
  ...
  await admin.from("site_settings").upsert({ key: KEY, value }, { onConflict: "key" });
}
...
export async function POST(request: Request) {
  ...
  if (!file?.size || !Number.isInteger(index) || index < 0 || index >= SLOT_COUNT) {
    return NextResponse.json({ error: "Invalid file or index (0-11)" }, { status: 400 });
  }
  const rawExt = file.name.split(".").pop()?.toLowerCase() || "";
  const ext = /^[a-z0-9]+$/.test(rawExt) ? rawExt : "jpg";
  const buf = Buffer.from(await file.arrayBuffer());
  const path = `immigration-projects/${index}.${ext}`;
