...
async function saveImages(images: string[]) {
  ...
  await admin.from("site_settings").upsert({ key: "home_visa_services_images", value }, { onConflict: "key" });
}

/** 后台：获取 6 张签证服务卡片图片（前 4 张=首页，6 张=签证页含超级签证、毕业工签） */
...
export async function POST(request: Request) {
  ...
  const path = `visa-services/${index}.${ext}`;
